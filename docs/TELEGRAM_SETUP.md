...
## Дополнение этапа 5 (09.09.2026)

В существующем проекте применены миграции расчётов, точных прав и FK-индексов;
...
Реальная Telegram-приёмка этапов 4–5 подтверждена пользователем 10.09.2026.
...
## Дополнение этапа 7 (14.09.2026)

В существующем проекте применена миграция
`20260914103425_resilience_history`; Edge Function `events` опубликована как
версия 6. Новых проектов и секретов нет. `events` по-прежнему использует
`verify_jwt=false`, потому что проверяет собственный app-session token внутри
RPC до и после ожидания прикладной блокировки.

Frontend не сохраняет Telegram initData или app-session token. HTTP 401 запускает
контролируемый повторный Telegram-вход, сохраняет форму и не повторяет мутацию
автоматически. Новая история, хранение черновиков и чек-лист реальной приёмки:
[RESILIENCE.md](RESILIENCE.md). Frontend Pages не публиковался без отдельного
указания пользователя.

## Дополнение этапа 8 (14.09.2026)

Все Edge Functions этапа 8 возвращают безопасный заголовок версии backend;
после публикации их проверяет read-only команда `pnpm smoke -- <project-url>`.
Новых секретов нет. Полный перечень имён секретов, backup/restore runbook,
матрица iOS/Android/Desktop и сценарий мероприятия A/B/C находятся в
[PILOT.md](PILOT.md). До фактического restore drill и физических проверок этап
имеет статус «технически подготовлен к пилотной приёмке», а не «завершён».
