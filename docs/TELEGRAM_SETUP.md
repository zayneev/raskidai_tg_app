...
Код готов к подключению. Этап считается завершённым после реального входа
через Telegram и проверки миграции в Supabase. Секреты не отправляйте в чат,
...
- До реального входа нужно добавить TELEGRAM_BOT_TOKEN и ALLOWED_ORIGINS
  в Edge Functions Secrets, затем настроить переменную GitHub Pages и публикацию.
